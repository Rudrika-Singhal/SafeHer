...
//       // Backend responded, but couldn't actually generate a message (e.g. it has no internet
//       // to reach Groq) — treat this the same as a failure, don't pretend it worked.
//       throw new Error('Backend could not generate a message (likely no internet on the server side)');
//     }
//     const finalMessage = data.message;
//     preview.textContent = finalMessage;
//     console.log('AI message source:', data.source); // "fallback-template" or "grok"
...
//       headers:{'Content-Type':'application/json'},
//       body: JSON.stringify({ contacts: numbers, message: finalMessage })
//     }).then(res=>res.json()).then(smsData=>({ smsData, finalMessage }));
//   })
//   .then(({smsData, finalMessage})=>{
//     console.log('SMS result:', smsData);
//     if(smsData && smsData.sent === true){
//       // Genuinely delivered via Twilio
//       lastFailedSOSKind = null;
//       document.querySelectorAll('#sos-sent-to .contact-sent span').forEach(s=> s.textContent = 'Delivered · SMS');
//     }else{
//       // Backend reachable, but SMS wasn't actually sent (Twilio not configured, or it failed server-side)
//       document.querySelectorAll('#sos-sent-to .contact-sent span').forEach(s=> s.textContent = smsData?.note || 'Not sent — SMS service unavailable');
//       handleSOSFailure(kind, name, loc, time);
//     }
//   })
//   .catch(err=>{
...
let nearbyOrigin = { lat: null, lng: null };
let nearby2Map = null;
let routeToPlaceLayer = null;
...
function renderPlaceList(containerId, places, icon, emptyLabel){
  ...
  el.innerHTML = places.slice(0,4).map((p,i)=>`
    <div class="contact-sent" style="cursor:pointer;" data-lat="${p.lat}" data-lng="${p.lng}" data-name="${p.name.replace(/"/g,'&quot;')}">
      <div class="av">${icon}</div><div><b>${p.name}</b><span id="${containerId}-status-${i}">${p.distance.toFixed(1)} km away · tap for route</span></div>
    </div>
  `).join('');

  // Wire up click handlers to draw a real route to whichever place is tapped
  el.querySelectorAll('.contact-sent[data-lat]').forEach(item=>{
    item.addEventListener('click', ()=>{
      ...
      showRouteToPlace(destLat, destLng, destName, statusSpan);
    });
  });
}

// Draws a real driving/walking route from the user's current search origin to a tapped place
async function showRouteToPlace(destLat, destLng, destName, statusSpan){
  if(!nearby2Map || nearbyOrigin.lat === null) return;
  const originalText = statusSpan.textContent;
  statusSpan.textContent = 'Finding route…';

  try{
    const url = `https://router.project-osrm.org/route/v1/driving/${nearbyOrigin.lng},${nearbyOrigin.lat};${destLng},${destLat}?overview=full&geometries=geojson`;
    const res = await fetch(url);
    const data = await res.json();
    ...
    if(routeToPlaceLayer) nearby2Map.removeLayer(routeToPlaceLayer);
    routeToPlaceLayer = L.polyline(coords, { color:'#E85D75', weight:5 }).addTo(nearby2Map);
    nearby2Map.fitBounds(routeToPlaceLayer.getBounds(), { padding:[24,24] });

    statusSpan.textContent = `${minutes} min · ${km} km · route shown on map`;
    toast(`Route to ${destName} shown on map`);
  }catch(err){
    console.warn('Could not fetch route to place:', err.message);
    statusSpan.textContent = originalText;
    toast("Couldn't find a route — try again");
  }
}
...
async function loadNearbyPlaces(map, lat, lng, markerLabel){
  ...
  // Clear previously drawn markers/routes so searching a new location doesn't stack old ones
  nearby2Layers.forEach(l=>map.removeLayer(l));
  nearby2Layers = [];
  if(routeToPlaceLayer){ map.removeLayer(routeToPlaceLayer); routeToPlaceLayer = null; }
